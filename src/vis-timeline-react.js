...
each(events, event => {
	;(eventPropTypes[event] = PropTypes.func), (eventDefaultProps[`${event}Handler`] = noop)
})
...
export default class Timeline extends Component {
	...
	init() {
		const { items, groups, options, selection, selectionOptions = {}, customTimes, animate = true, currentTime } = this.props
		...
		if (groups.length > 0) {
			// @see: https://visjs.github.io/vis-data/data/datapipe.html#TypeCoercion
...
Timeline.propTypes = assign(
	{
		...
		currentTime: PropTypes.oneOfType([PropTypes.string, PropTypes.instanceOf(Date), PropTypes.number]),
	},
	eventPropTypes
)
